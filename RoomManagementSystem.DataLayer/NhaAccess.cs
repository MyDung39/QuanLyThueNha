...
namespace RoomManagementSystem.DataLayer
{
    public class NhaAccess
    {
        ...
        //Lay so phong
        public int getSoPhong(string MaNha)
        {
            ...
            var result = db.ExecuteScalar(qr, parameters);
            return result != null ? Convert.ToInt32(result) : 0;
        }
        //Lay so phong hien tai
        public int getSoPhongHienTai(string MaNha)
        {
    ...
    /*    public Boolean updateHouse(string MaNha, string DiaChi, string GhiChu)
        {
            ...
            //int SoPhong = getSoPhong(MaNha);
            //int TongSoPhongHienTai = getSoPhongHienTai(MaNha);
    ...
    */
        ...
        public Boolean updateHouse(string MaNha, string DiaChi, string GhiChu)
        {
            // Chúng ta chỉ cập nhật những gì được chỉnh sửa từ popup
            string qr = @"UPDATE Nha SET 
                            DiaChi = @DiaChi,
                            GhiChu = @GhiChu,
                            NgayCapNhat = GETDATE()
                          WHERE MaNha = @MaNha";
