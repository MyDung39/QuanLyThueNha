...
namespace RoomManagementSystem.DataLayer
{
    public class HopDongDAL
    {
        ...
        public List<HopDong> GetAllHopDong()
        {
            ...
            foreach (DataRow reader in dt.Rows)
            {
                HopDong hd = new HopDong()
                {
                    ...
                    default : Convert.ToDateTime(reader["NgayKetThuc"]),
        ...
        public HopDongXemIn? GetInHD(string maHopDong)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                HopDongXemIn hdXemIn = new HopDongXemIn()
                {
                    ...
                    // THÊM DÒNG NÀY ĐỂ ĐỌC SỐ PHÒNG
                    TongSoPhong = reader["TongSoPhong"] == DBNull.Value ?
                    0 : Convert.ToInt32(reader["TongSoPhong"]),
        ...
        // <-- THÊM MỚI (Cho Yêu cầu 3: Lấy tất cả thông báo)
        public List<ThongBaoHan> GetAllThongBaoHan()
        {
            List<ThongBaoHan> ds = new List<ThongBaoHan>();
            string qr = "SELECT MaThongBao, MaHopDong, NoiDung, NgayThongBao, TrangThai FROM ThongBaoHan";
            DataTable dt = db.ExecuteQuery(qr);

            foreach (DataRow row in dt.Rows)
            {
                // Giả định lớp ThongBaoHan có các thuộc tính này
                ds.Add(new ThongBaoHan
                {
        ...
        public HopDong? GetActiveContractByTenantId(string maNguoiThue)
        {
            ...
            string qr = @"SELECT TOP 1 h.* FROM HopDong h
            ...
            // Lấy hợp đồng mới nhất nếu có nhiều
        ...
        // Dùng cho giao dịch (Cần cho việc cập nhật)
